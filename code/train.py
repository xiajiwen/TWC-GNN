...
def train(epoch):
    ...
    return loss_val.data.item(), loss_train.data.item()
...
for epoch in range(args.epochs):
    loss_val, loss_train = train(epoch)
    loss_values.append(loss_val)

    loss_list.append(loss_train)  # 画图
...
loss_our = pd.DataFrame(data=loss_list)  # 画图
loss_our.to_csv('loss_our_pubmed.csv')  # 画图
...
# Testing
# model.load_state_dict(torch.load('./30%pubmed_72.9%.pkl'))
compute_test()
